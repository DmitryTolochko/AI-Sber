...
@pytest.fixture
def mock_dictionary():
    """Return mock dictionary data for testing."""
    return [
        {
            "original": "со̄кӣни",
...
@pytest.fixture
def mock_sentences():
    """Return mock sentences data for testing."""
    return [
        {
...
def russian_texts_for_several_translation():
    """Return test data for multiple translation attempts."""
    return [
        {
...
def test_get_word_status_code(mock_dictionary):
    """Test that get-word endpoint returns 200 status code."""
...
def test_get_word_status_code_reverse(mock_dictionary):
    """Test that get-word endpoint returns 200 for reverse lookup."""
    dictionary = mock_dictionary
    for pair in dictionary:
...
def test_get_word_original(mock_dictionary):
    """Test that get-word returns correct original word."""
    dictionary = mock_dictionary
    for pair in dictionary:
        response = client.get(f"/dictionary/get-word?word={pair['original']}")
        data = response.json()
...
def test_get_word_original_reverse(mock_dictionary):
    """Test that get-word returns correct original for reverse lookup."""
    dictionary = mock_dictionary
    for pair in dictionary:
        response = client.get(f"/dictionary/get-word?word={pair['translation']}")
        data = response.json()
...
def test_get_word_translation(mock_dictionary):
    """Test that get-word returns correct translation."""
    dictionary = mock_dictionary
    for pair in dictionary:
        response = client.get(f"/dictionary/get-word?word={pair['original']}")
        data = response.json()
...
def test_get_word_translation_reverse(mock_dictionary):
    """Test that get-word returns correct translation for reverse lookup."""
    dictionary = mock_dictionary
    for pair in dictionary:
        response = client.get(f"/dictionary/get-word?word={pair['translation']}")
        data = response.json()
...
def test_sentences_code(mock_dictionary):
    """Test that sentences endpoint returns 200 status code."""
    dictionary = mock_dictionary
    for pair in dictionary:
...
def test_sentences_searched_word(mock_dictionary):
    """Test that sentences returns correct searched word."""
    dictionary = mock_dictionary
    for pair in dictionary:
        response = client.get(f"/dictionary/sentences?word={pair['original']}")
        data = response.json()
...
def test_sentences_matches(mock_dictionary):
    """Test that sentences returns matching sentences."""
    dictionary = mock_dictionary
    for pair in dictionary:
        response = client.get(f"/dictionary/sentences?word={pair['original']}")
        data = response.json()
...
def test_sentences_matches_reverse(mock_dictionary):
    """Test that sentences returns matching sentences for reverse lookup."""
    dictionary = mock_dictionary
    for pair in dictionary:
...
def test_translation_to_russian_status_code(mock_sentences):
    """Test that to-russian endpoint returns 200 status code."""
    sentences = mock_sentences
    for pair in sentences:
        response = client.get(
            f"/translation/to-russian?nanai_text={pair['original']}&attempt=1"
        )
        assert response.status_code == 200


def test_translation_to_russian(mock_sentences):
    """Test that to-russian returns correct translation."""
...
def test_translation_to_russian_not_return_original(random_nanai_texts):
    """Test that to-russian does not return original text."""
    attempts = [1, 2, 3]
    for word in random_nanai_texts:
        for attempt in attempts:
            response = client.get(
                f"/translation/to-russian?nanai_text={word}&attempt={attempt}"
            )
...
def test_translation_to_nanai_status_code(mock_sentences):
    """Test that to-nanai endpoint returns 200 status code."""
    sentences = mock_sentences
    for pair in sentences:
        response = client.get(
            f"/translation/to-nanai?russian_text={pair['translation']}&attempt=1"
        )
        assert response.status_code == 200


def test_translation_to_nanai_several_attempts(russian_texts_for_several_translation):
    """Test that to-nanai returns correct translation for several attempts."""
    for pair in russian_texts_for_several_translation:
        response = client.get(
            f"/translation/to-nanai?"
            f"russian_text={pair['russian_original']}&attempt={pair['attempt']}"
        )
        assert pair['nanai_translation'] == response.json()['text_to_translated']


def test_translation_to_nanai_several_attempts_different_results(
    russian_texts_for_several_translation
):
    """Test that different attempts produce different results."""
    ...
    for pair in russian_texts_for_several_translation:
        ...
        sentences_dictionary[pair["russian_original"]][pair["attempt"]] = (
            result["text_to_translated"]
        )
...
def test_translation_to_nanai_not_return_original(random_russian_texts):
    """Test that to-nanai does not return original text."""
    attempts = [1, 2, 3]
    for word in random_russian_texts:
        for attempt in attempts:
            response = client.get(
                f"/translation/to-nanai?russian_text={word}&attempt={attempt}"
            )
            assert word != response.json()['text_to_translated']
